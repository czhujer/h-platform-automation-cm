...
nginx::resource::upstream { 'jenkins-http':
  members => [
    '127.0.0.1:8080',
  ],
}
...
nginx::resource::server { "${facts['ipaddress']}":
  ...
  ssl_protocols       => 'TLSv1.2 TLSv1.3',
  #
  proxy_redirect       => "http://127.0.0.1:8080 https://${facts['ipaddress']}",
  #
  add_header => {
      #because: better security
...
#nginx::resource::location { "sf-sh-s11.sugarfactory.cz_status2":
#     ensure          => present,
#     ssl             => true,
#     ssl_only        => true,
#     vhost           => "sf-sh-s11.sugarfactory.cz",
#     www_root        => "/var/www/html",
#     location        => '/nginx_vts_status',
#     proxy           => undef,
#     #stub_status      => true,
#     location_allow   => ['127.0.0.1', '172.16.0.0/12', '10.211.1.84'],
#     location_deny    => ['all'],
#     location_cfg_append => {
#         'access_log' => 'off',
#         # vts module params
